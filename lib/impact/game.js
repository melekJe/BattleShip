...
ig.Game = ig.Class.extend({
	...
	loadLevel2: function( data, coordx, coordy , matrice) {
		this.screen = {x: 0, y: 0};
		
		for( var i = 0; i < data.layer.length; i++ ) {
			var cases = data.layer[i];
		        cases.data = matrice;
		}
		...
		for( var i = 0; i < data.entities.length; i++ ) {
			var ent = data.entities[i];
			ent.x = coordx[i]*24;
			//console.log("I'm in game: I'm x",coordx[i],",I'm y",coordy[i]);
			ent.y = coordy[i]*24;
			var ente = ent.settings;
			this.spawnEntity( ent.type, ent.x, ent.y, ent.settings );
		}
		this.sortEntities();
	...
	loadLevel3: function( data, matrice ) {
		this.screen = {x: 0, y: 0};
